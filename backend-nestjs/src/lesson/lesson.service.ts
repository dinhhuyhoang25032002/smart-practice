...
export class LessonService {
  ...
  async handleGetLessonById(
    ...
    if (role === UserRole.STUDENT) {
      const startTimeData = await this.startTimeModel
        .findOne({
          lessonId,
          studentId: userId,
        })
      ...
      const { idFrontLesson, _id } = await this.lessonModel
        .findById(lessonId)
        .exec();
      ...
      const startTime = await this.startTimeModel
        .findOne({ studentId: userId, lessonId: idFrontLesson })
        .lean()
        .exec();
  ...
  async handleCreateLesson(contentLesson: LessonDto) {
    ...
    if (!courseData) {
      return {
        message: 'Không tồn tại khóa học để thêm.',
        status: HttpStatus.NOT_FOUND,
      };
    }

    const isLessonExisted = await this.lessonModel
      .findOne({ name: name })
      .lean()
      .exec();
    if (isLessonExisted) {
  ...
  async handleGetAllSections(lessonId: string, userId: string) {
    ...
    const timeview = await this.timeviewModel
      .find({
        lessonId,
        userId,
    ...
    const data = indexItem.map((item) => {
      const map = record.find(
        (recordItem) => recordItem.sectionId === item._id.toString(),
      );
      return {
        ...item,
        views: map?.views ?? 0,
