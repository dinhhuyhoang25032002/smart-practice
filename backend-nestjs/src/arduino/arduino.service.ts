...
export class ArduinoService {
  ...
  async createBuild(body: CompileCodeDto) {
    ...
    if (!this.ALLOWED_BOARDS[board]) {
      ...
      throw new BadRequestException(
        `Board '${board}' is not supported. Supported boards: ${Object.keys(this.ALLOWED_BOARDS).join(', ')}`,
      );
    }
    ...
    try {
      ...
      if (!hexFile) {
        console.log(
          `[LOG] [createBuild] Không tìm thấy file .${boardInfo.hexExtension} sau khi biên dịch.`,
        );
        throw new Error(
          `Không tìm thấy file .${boardInfo.hexExtension} sau khi biên dịch.\nFiles in output dir: ${files.join(', ')}\nSTDOUT: ${result.stdout}\nSTDERR: ${result.stderr}`,
        );
      }
      // Lấy kích thước file
      ...
      try {
        const fileContent = await fs.readFile(hexFilePath, {
          encoding: 'utf8',
        });
        console.log(
          '[LOG] [createBuild] 100 ký tự đầu file:',
          fileContent.slice(0, 100),
        );
      } catch (e) {
        console.warn(
          '[LOG] [createBuild] Không đọc được nội dung file output:',
          e,
        );
      }
      await this.uploadToArduino({
        hexFile: `${outputDir}/${mainFileName}`,
        fqbn: board,
        port: '/dev/ttyUSB0', // Cổng mặc định, có thể thay đổi theo yêu cầu
      ...
      try {
        ...
        console.log(
          '[LOG] [createBuild] Đã xóa thư mục tạm:',
          sketchFolderPath,
        );
      } catch (e) {
  ...
  async getBuildInfo(buildId: string) {
    ...
    try {
      ...
      if (!file) {
        file = files.find(
          (f) => f.endsWith('.hex') && f.includes('with_bootloader'),
        );
        if (file)
          console.log(
            '[LOG] [getBuildInfo] Chọn file with_bootloader.hex:',
            file,
          );
      }
      ...
      if (!file) {
        console.log(
          '[LOG] [getBuildInfo] Không tìm thấy file hex/bin cho buildId:',
          buildId,
        );
      ...
      const boardInfo = Object.entries(this.ALLOWED_BOARDS).find(
        ([_, info]) => info.hexExtension === fileExtension,
      );
      console.log(
        '[LOG] [getBuildInfo] Trả về file:',
        file,
        'downloadUrl:',
        `/api/arduino/builds/${buildId}/${fileExtension}`,
      );
      return {
        buildId,
        fileName: file,
  ...
  private async uploadToArduino(options: {
    hexFile: string;
    port: string;
    fqbn: string;
    baudRate: number;
  }) {
    console.log(options.hexFile);
    const arduinoCli = path.join(process.cwd(), 'bin', 'arduino-cli');
    const uploadCommand = `${arduinoCli} upload -p ${options.port} --fqbn ${options.fqbn} -i ${options.hexFile}`;

    try {
      await execPromise(uploadCommand);
      console.log('[LOG] [uploadToArduino] Upload successful');
    } catch (error) {
      console.error('[LOG] [uploadToArduino] Upload failed:', error);
      throw new Error('Failed to upload to Arduino: ' + error.message);
    }
  }
  ...
  async getBuildFile(
    ...
    console.log(
      `[LOG] [getBuildFile] buildId: ${buildId}, fileType: ${fileType}`,
    );
    if (!/^[a-fA-F0-9-]{36}$/.test(buildId)) {
    ...
    try {
      ...
      // Ưu tiên file hex chính
      let hexFile = files.find(
        (f) => f === `sketch_${buildId}.ino.${fileType}`,
      );
      if (hexFile) {
      ...
      if (!hexFile && fileType === 'hex') {
        ...
        if (hexFile)
          console.log(
            '[LOG] [getBuildFile] Chọn file with_bootloader.hex:',
            hexFile,
          );
      }
      // Nếu vẫn không có, lấy file .bin
      if (!hexFile && fileType === 'bin') {
        ...
        if (hexFile)
          console.log('[LOG] [getBuildFile] Chọn file .bin:', hexFile);
      }
      if (!hexFile) {
        console.log(
          `[LOG] [getBuildFile] Không tìm thấy file .${fileType} cho buildId ${buildId}`,
        );
        throw new NotFoundException(
          `File .${fileType} not found for build ID '${buildId}'.`,
        );
      }
