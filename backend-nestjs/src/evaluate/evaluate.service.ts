...
export class EvaluateService {
    ...
    async handleCreateAEvaluation(id: string, data: EvaluateDto) {
        ...
        if (!result) {
            return new NotFoundException("Không tìm thấy dữ liệu về bài làm của sinh viên")
        }

        const evaluateData = await this.evaluateModel.findOne({ teacherId: id, lessonId: lessonId, studentId: studentId }).exec()
        if (evaluateData) {
